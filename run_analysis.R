...
runAnalysis <- function() {
  ...
  MyTidyWay <- ddply(tidy, .(Subject, Activity), limitedColMeans)
  names(MyTidyWay)[-c(1,2)] <- paste0("Mean", names(MyTidyWay)[-c(1,2)])

  # Write file
  write.table(MyTidyWay, "MyTidyWay.txt", row.names = FALSE)

  # Also return data
  MyTidyWay
}

# Use to check that the MyTidyWay.txt is properly readable
checkData <- function() {
  read.table("MyTidyWay.txt", header = TRUE)
}
